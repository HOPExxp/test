...
try:
    res = simulate_fmu(
        ...
        output=['supplyTemp', 'returnTemp','singleChiller.singleChiller.chillerWithHex2_1.chi.P'],
        input=input_signals,
        validate=False
    )
    ...
    plt.plot(res['time'], res['supplyTemp'] - 273.15, label='Supply Temperature [°C]', color='tab:blue')
    plt.plot(res['time'], res['returnTemp'] - 273.15, label='Return Temperature [°C]', color='tab:red')
    ...
    res = simulate_fmu(
        ...
        stop_time=STOP_TIME,
        output_interval=STEP_SIZE,
        output=['supplyTemp', 'returnTemp'],
        validate=False
    )

    # 保存和绘图代码同上
# Coolingstation[end].singleChiller.singleChiller.chillerWithHex2_1.chi.P
# singleChiller.singleChiller.chillerWithHex2_1.chi.P