...
class CoolingSystemSimulator:
    def __init__(self, fmu_path):
        ...
        self.original_csv_data = None  # 新增：存储原始CSV数据用于调试
    ...
    def create_input_signals(self, start_time, stop_time, step_size, input_values=None, input_csv_path=None):
        ...
        print(f"生成的时间序列: 从 {start_time} 到 {stop_time}, 步长 {step_size}, 共 {len(time_points)} 个点")
        ...
        if input_values:
            ...
            print(f"使用自定义输入值: {input_values}")

        # 从CSV文件读取输入数据（如果提供了路径）
        csv_data = {}
        self.original_csv_data = None  # 重置原始CSV数据
        if input_csv_path and os.path.exists(input_csv_path):
            try:
                df = pd.read_csv(input_csv_path)
                self.original_csv_data = df  # 保存原始数据用于调试
                print(f"成功读取CSV文件: {input_csv_path}, 包含 {len(df)} 行数据")
                print(f"CSV文件中的列: {df.columns.tolist()}")
                print(f"CSV时间范围: 从 {df['time'].min()} 到 {df['time'].max()}")

                if 'time' not in df.columns:
                    print("警告: CSV文件中没有'time'列，无法读取时间序列数据")
                    return self._create_default_signals(time_points, default_values)

                # 检查CSV时间范围与仿真时间范围是否匹配
                if df['time'].max() < stop_time:
                    print(f"警告: CSV文件的最大时间({df['time'].max()})小于仿真结束时间({stop_time})")
                if df['time'].min() > start_time:
                    print(f"警告: CSV文件的最小时间({df['time'].min()})大于仿真开始时间({start_time})")
                ...
                for var in self.input_vars:
                    ...
                    if var_name in df.columns:
                        # 检查该变量是否有变化，还是恒定值
                        if len(df[var_name].unique()) > 1:
                            print(f"变量 {var_name} 在CSV中有变化，将进行插值处理")
                        else:
                            print(f"变量 {var_name} 在CSV中是恒定值: {df[var_name].iloc[0]}")
                        ...
                        interp_func = interp1d(
                            ...
                            df[var_name],
                            kind='linear',
                            bounds_error=False,  # 超出范围时不抛出错误
                            fill_value=(df[var_name].iloc[0], df[var_name].iloc[-1])  # 用首尾值填充超出范围的部分
                        )
                        #     bounds_error=True,  # 超出范围时抛出错误而非填充
                        #     fill_value="extrapolate"  # 超出范围时外推而非使用首尾值
                        # )
                        ...
                        try:
                            ...
                            # 验证数据
                            if len(csv_data[var_name]) != len(time_points):
                                print(f"警告: 变量 {var_name} 插值后长度不匹配: {len(csv_data[var_name])} vs {len(time_points)}")
                            else:
                                print(f"成功为变量 {var_name} 生成 {len(time_points)} 个插值点")
                        except ValueError as e:
                            print(f"变量 {var_name} 插值失败: {e}，将使用默认值")
                            csv_data[var_name] = np.full_like(time_points, default_values.get(var_name, 0))
                    else:
                        print(f"CSV文件中没有找到变量 {var_name}，将使用默认值")
            except Exception as e:
                print(f"读取CSV文件时出错: {e}，将使用默认值")
                return self._create_default_signals(time_points, default_values)
        elif input_csv_path:
            print(f"警告: 未找到CSV文件 {input_csv_path}，将使用默认值")
        return self._create_default_signals(time_points, default_values)
        ...
        for var in self.input_vars:
            ...
            print(f"变量 {var_name} 使用 {source}，值范围: {np.min(values)} 到 {np.max(values)}")
    ...
    def _create_default_signals(self, time_points, default_values):
        """创建仅使用默认值的输入信号（用于错误处理）"""
        input_signals = []
        dtype_list = [('time', np.float64)]
...
if __name__ == "__main__":
    ...
    if success:
        ...
        plt.plot(simulator.results['time'], simulator.results['supplyTemp'] - 273.15, label='Supply Temperature [°C]', color='tab:blue')
        plt.plot(simulator.results['time'], simulator.results['returnTemp'] - 273.15, label='Return Temperature [°C]', color='tab:red')
